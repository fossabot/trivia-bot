...
async def serverleaderboard(ctx):
    with open('data.txt') as json_file:
        ...
        data = json.load(json_file)
        datalist = data.items()
        for id in message.guild.members:
            id = id.id
            for globaluser in datalist:
               if globaluser[0] == id:
                   serverlist += globaluser
            
        sorteddata = sorted(serverlist,key=itemgetter(1),reverse=True)
        try:
            firstuserid = sorteddata[0][0]
        except:
            firstuserid = "null"
